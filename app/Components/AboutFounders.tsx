...
const AboutFounders = () => {
  ...
  <p className='text-gray-200 mb-[5vh]'>Meet The Founders</p>
  
  <div className='flex flex-col md:flex-row text-white gap-[5vw] mx-4  mt-5 '>

            {
                founders.map((founder, index) => (
                    ...
                    <>
                     ...
                     <div className='md:w-[36vw] lg:w-[24vw] relative w-[60vw]  mt-5  shadow-spotlight  bg-[#121212B2]  border-[#2A2427] border-solid border-1 transition-transform duration-300 h-[18rem]  rounded-[18px] flex flex-col p-6'>
                        <div className='absolute mt-[2px] text-[1.6rem] text-[#FF7B15]'>{founder.name}</div>
                        <div className='absolute mt-[38px] text-[0.6rem] text-gray-500'>{founder.intro}</div>
                        {
                         <>
            ...
            <div className='flex flex-col gap-2 mt-[5vh]'>
                {
                    founder.about.map((each, index) => (
                        <div key={index} className='flex gap-4  text-wrap items-center text-[0.9rem] text-gray-300'>
                            {
                                each.icon
                            }
                            <p>{each.point}</p>
